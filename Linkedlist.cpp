/*
Changes still needed to be applied
(2) Playback Feature (Pausing and Resuming Music)

(3) Maintaining Current Song even after shuffling the playlist



(5) Searching and Sorting Songs

(6) Calculation of Total Runtime (Album / Song List)

(7) Selecting a Song to Play
(Use _kbhit() to navigate through the playlist and press Enter to select)
*/
// windows ver only works on vscode
#include <iostream>
#include <fstream>
#include <string>
#include <cstdlib>
...
private:
    ...
    void rewriteCSV() {
        ofstream ofs("playlist_temp.csv");
        if (!ofs.is_open()) return;
        
        // Write header
        ofs << EXPECTED_HEADER << "\n";
        
        // Write all songs except deleted one
        if (head) {
            Node* temp = head;
            do {
                ofs << temp->song.title << "," 
        ...
        ofs.close();
        remove("playlist.csv");
        rename("playlist_temp.csv", "playlist.csv");
    }
...
public:
   ...
   void deleteSong() {
    ...
    showPlaylist();  // Show current playlist first
    ...
    if (songNum < 1 || songNum > countSongs()) {
    ...
    // Store song info for confirmation
    string title = temp->song.title;
    string artist = temp->song.artist;
    ...
    if (temp->next == temp) {
        ...
        // Update head if deleting head
        if (temp == head) head = temp->next;
        
        // Update current if deleting current
        if (temp == current) {
            current = temp->next;
            isPlaying = false;
        }
    ...
    // Rewrite CSV without deleted song
    rewriteCSV();
    
    cout << "\n[+] Deleted: \"" << title << "\" by " << artist << "\n";
}
    // 2. Play Current Song
    void playSong() 
    {
...
{
    clearScreen();
    cout << "=== Create Custom Playlist ===\n\n";
    
    showPlaylist();  // Show all available songs first
    
    if (!head) 
    {
        cout << "\n[!] Main playlist is empty. Add songs first!\n";
        cout << "Press Enter to go back...";
        cin.get();
        return;
    }
    
    vector<Song> customSongs;
    vector<int> selectedIndices;
    int totalSongs = countSongs();
    char addMore;
    
    cout << "\nSelect songs for your custom playlist (by number):\n\n";

    do 
    {
        cout << "Enter song number (1-" << totalSongs << "): ";
        int songNum;
        cin >> songNum;
        cin.ignore();
        ...
        if (alreadySelected) 
        {
            cout << "[!] Song already in custom playlist!\n\n";
            continue;
        }
        ...
        customSongs.push_back(temp->song);
        selectedIndices.push_back(songNum);
        
        cout << "[+] Added: \"" << temp->song.title 
             << "\" by " << temp->song.artist << "\n\n";
    ...
    if (customSongs.empty()) 
    {
        cout << "[!] No songs selected. Returning...\n";
        sleepOneSecond();
        return;
    }

    // Create TEMPORARY custom playlist (doesn't modify main playlist/CSV)
    cout << "\n=== YOUR CUSTOM PLAYLIST ===\n\n";
    cout << "Custom playlist created with " << customSongs.size() << " songs:\n\n";
    
    for (size_t i = 0; i < customSongs.size(); i++) 
    {
        cout << (i+1) << ". \"" << customSongs[i].title 
             << "\" by " << customSongs[i].artist 
             << "  |  " << customSongs[i].duration << "\n";
    }
    
    cout << "\n🎵 Now playing your custom playlist! 🎵\n\n";
    sleepOneSecond();
    ...
    {
        clearScreen();
        cout << "=== Custom Playlist ===" << "  (" << (i+1) << "/" << customSongs.size() << ")\n\n";
        
        displaySong(customSongs[i]);
        cout << "\n  -- Custom Playlist Mode --\n";
        cout << "  (Press any key to skip to next song)\n\n";
        
        runTimer(customSongs[i]);
        
        if (i < customSongs.size() - 1) 
        {
...
void menu() {
    ...
    do {
        ...
        cout << "7. Play Random Song\n";
        cout << "8. Shuffle Playlist\n";
        cout << "9. Custom Playlist\n";
        cout << "0. Exit\n";
        ...
        switch (choice) {
            ...
            case 2: player.deleteSong();      break;
            case 3: player.playSong();        break;
            case 4: player.nextSong();        break;
            case 5: player.prevSong();        break;
            case 6: player.showPlaylist();    break;
            case 7: player.playRandom();      break;
            case 8: player.shufflePlaylist(); break;
            case 9: player.customplaylist();  break;
            case 0:
                clearScreen();
